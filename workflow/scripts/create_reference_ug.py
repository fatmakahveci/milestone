...
-----------------------------------------------
Authors: @fatmakhv
The latest update: September 24, 2021
-----------------------------------------------
"""
...
def get_cds_list() -> list:
	...
	for file_name in os.listdir(args.genome_dir):
...
if __name__ == "__main__":
	...
	try:
		...
		Path(f"{args.genome_dir}/references").mkdir(exist_ok=True)

		# create FASTA directory for each core gene's alleles
		Path(f"{args.genome_dir}/alleles").mkdir(exist_ok=True)

	except OSError:

		print(f"Creation of the directories {args.genome_dir}/references "
							f"and {args.genome_dir}/alleles are failed.")
	...
	for cds in cds_list:
		...
		create_cds_list(f"{args.genome_dir}", cds, cds_to_merge_list, {args.threads})

	create_reference_vcf_fasta(f"{args.genome_dir}", cds_to_merge_list)
	...
	# os.system(f"rm -rf {args.genome_dir}")