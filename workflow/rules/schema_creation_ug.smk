...
rule create_reference_vcf_fasta:
    ...
    shell:
        ...
        echo "create_reference.py is running on{input.genome_dir}." | tee -a {params.log_file}
        echo "Output files '{output.reference_vcf}', and '{output.reference_fasta}' are created. " | tee -a {params.log_file}
        pwd
        echo "---------------------------------------" | tee -a {params.log_file}
        python {input.code_dir}/scripts/create_reference_ug.py --genome_dir {input.genome_dir} --reference_vcf {output.reference_vcf} --reference_fasta {output.reference_fasta} --reference_info {params.reference_info_txt} --threads {threads} 2>&1 | tee -a {params.log_file}
        now=$(date +"%T")
